...
export default function TotalSection({ expenses }) {
  ...
  const balance = totals.lend - totals.debt;
    ...
    <div className="bg-white rounded-lg shadow-md p-4 mb-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-green-50 rounded-lg p-4">
          <h3 className="text-sm font-medium text-green-800">Total Lend</h3>
          <p className="text-2xl font-bold text-green-600">₹{totals.lend.toFixed(2)}</p>
        </div>

        {/* Debt Total */}
        <div className="bg-red-50 rounded-lg p-4">
          <h3 className="text-sm font-medium text-red-800">Total Debt</h3>
          <p className="text-2xl font-bold text-red-600">₹{totals.debt.toFixed(2)}</p>
        </div>
